# Lab Tk-5: Menús, diálogos y eventos
...
class AppHMI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title('HMI — Interfaces y Puertos')
    ...
    def _construir(self):
        ...
        m_archivo.add_command(label='Nueva sesión',    command=self.nueva_sesion,   accelerator='Ctrl+N')
        m_archivo.add_command(label='Abrir config…',  command=self.abrir_config,   accelerator='Ctrl+O')
        m_archivo.add_command(label='Guardar config…',command=self.guardar_config, accelerator='Ctrl+S')
        m_archivo.add_separator()
        m_archivo.add_command(label='Exportar CSV…',  command=self.exportar_csv,   accelerator='Ctrl+E')
        m_archivo.add_separator()
        ...
        m_ayuda.add_command(label='Documentación',
            command=lambda: os.startfile('https://tkdocs.com') if hasattr(os,'startfile')
                else __import__('webbrowser').open('https://tkdocs.com'))
        ...
        self.log = scrolledtext.ScrolledText(self.frame_log, height=5,
            bg='#0f172a', fg='#64748b', font=('Courier New',8))
    ...
    def guardar_config(self):
        ...
        if ruta:
            ...
            with open(ruta,'w') as f: json.dump(cfg, f, indent=2)
    ...
    def exportar_csv(self):
        ...
        if ruta:
            with open(ruta,'w') as f:
                ...
                f.write('2025-01-01 00:00:00,23.4,1013.2,55.1\n')
            self._log(f'CSV exportado: {os.path.basename(ruta)}')

    def toggle_log(self):
        if self.var_log.get(): self.frame_log.pack(fill=tk.X, side=tk.BOTTOM)
        else: self.frame_log.pack_forget()
    ...
    def abrir_crc(self):
        ...
        def calcular():
            ...
            except: lbl_res.config(text='Error en los datos')
    ...
    def abrir_conversor(self):
        ...
        def convertir():
            ...
            except: lbl_res.config(text='Error en valores')
