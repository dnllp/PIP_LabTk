...
class HiloSerial(threading.Thread):
    ...
    def __init__(self, q_datos: queue.Queue, q_log: queue.Queue):
        super().__init__(daemon=True)   # daemon=True: muere con la app
...
class App:
    def __init__(self):
        ...
        # Hilo serial (inicia pero espera activo.wait())
        self.hilo = HiloSerial(self.q_datos, self.q_log)
        self.hilo.start()

        self._ui()
        self._poll()   # iniciar el loop de lectura de la queue
    ...
    def _ui(self):
        ...
        tk.Label(bar, text='Puerto:', bg='#111827', fg='#94a3b8',
            font=('Arial',9)).pack(side=tk.LEFT, padx=(12,2), pady=12)
        ttk.Combobox(bar, textvariable=self.var_pto, values=puertos, width=12
        ).pack(side=tk.LEFT, padx=2, pady=10)

        tk.Label(bar, text='Baud:', bg='#111827', fg='#94a3b8',
            font=('Arial',9)).pack(side=tk.LEFT, padx=(8,2))
        ttk.Combobox(bar, textvariable=self.var_baud,
            values=['9600','19200','57600','115200'], width=8
        ).pack(side=tk.LEFT, padx=2)

        self.btn_cx = tk.Button(bar, text='Conectar', command=self._conectar,
            bg='#1E5E2E', fg='white', font=('Arial',9,'bold'),
            relief=tk.FLAT, padx=12)
        self.btn_cx.pack(side=tk.LEFT, padx=10)

        self.lbl_cx = tk.Label(bar, text='● Desconectado',
            bg='#111827', fg='#ef4444', font=('Arial',9,'bold'))
        ...
        self.lbls = {}
        defs = [('temp','Temperatura','°C','#ef4444'),
        ...
        for i,(key,nombre,uni,color) in enumerate(defs):
            ...
            tk.Label(f, text=nombre, bg='#1e2736', fg='#94a3b8',
                font=('Arial',9)).pack(anchor='w', padx=10, pady=(8,0))
            lbl = tk.Label(f, text=f'-- {uni}', bg='#1e2736', fg=color,
        ...
        for txt, cmd in [('LED Verde ON','SET:LED:0:1'),('LED Verde OFF','SET:LED:0:0'),
                         ('LED Rojo ON','SET:LED:1:1'),('LED Rojo OFF','SET:LED:1:0'),
                         ('PING','PING')]:
            tk.Button(cmd_f, text=txt, command=lambda c=cmd: self.hilo.enviar(c),
                bg='#1F4E79', fg='white', font=('Arial',8), relief=tk.FLAT,
                padx=8, pady=4).pack(side=tk.LEFT, padx=4, pady=6)

        # ── Log ─────────────────────────────────────────────────
        self.log = scrolledtext.ScrolledText(
            self.root, height=6, bg='#0d1520', fg='#64748b',
    ...
    def _conectar(self):
        if self.hilo.activo.is_set():
            ...
            if self.hilo.conectar(self.var_pto.get(), int(self.var_baud.get())):
                self.btn_cx.config(text='Desconectar', bg='#922B21')

    # ── POLL: leer colas desde el hilo principal cada 100 ms ───
    def _poll(self):
        # 1. Leer mensajes de log
        while not self.q_log.empty():
            nivel, msg = self.q_log.get_nowait()
            self._log(nivel, msg)
            if nivel == 'INFO' and 'Conectado' in msg:
                self.lbl_cx.config(text='● Conectado', fg='#22c55e')
            elif nivel == 'INFO' and 'Desconectado' in msg:
                self.lbl_cx.config(text='● Desconectado', fg='#ef4444')
                self.btn_cx.config(text='Conectar', bg='#1E5E2E')
        ...
        self.root.after(100, self._poll)   # repetir en 100 ms
